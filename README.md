# MLOps Bootcamp — AutoGluon Air Quality Prediction
...
`data/raw/air_quality.csv` is a **synthetic** dataset (generated by
`scripts_generate_sample_data.py`) so the whole pipeline runs end-to-end
without needing a real data source. Swap it for your real air-quality data
— `data_ingestion.py` doesn't care where the CSV came from, only that it
matches the columns declared in `schema.yaml`.
